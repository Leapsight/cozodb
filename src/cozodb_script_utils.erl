...
**Parameters:**
- `Spec` :: `map()` - A relation specification map. It must include the keys
  `keys` and `columns`. If either is missing, they are defaulted to empty lists.
...
### Parameters
- `Spec` :: `map()` - An index specification map. The map must specify the
  index `type` (e.g. covering, hnsw, fts, or lsh) and include all required keys
  for that index type.
...
### Parameters
- `Specs` :: `[trigger_spec()]` - A list of trigger specifications, where each
specification is a tuple of the form `{Event, Script}`. Supported events include
`on_put`, `on_rm` (or `on_remove`), and `on_replace`.
...
-doc """
Converts a given term into a CozoScript–compatible string representation using
 the specified mode.

### Parameters:
- `Term` :: _any_ - The term to convert.
...
%% @end
%% -----------------------------------------------------------------------------
encode_index_entry(hnsw, K, N) when
    (K == dim orelse K == m orelse K == ef_construction) andalso
...
encode_index_entry(lsh, K, N) when
    (K == target_threshold orelse
        ...
        K == false_negative_weight) andalso
        is_float(N) andalso N > 0.0
->
    <<
